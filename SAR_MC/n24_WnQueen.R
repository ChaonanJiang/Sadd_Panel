...
n1 <- r2*r2   #n is 10000 
N1 <- n1*T 
W1 <- cell2nb(r2,r2,type = "queen",torus = F)    
...
n2 <- r3*r3   #n is 10000 
N2 <- n2*T 
W2 <- cell2nb(r3,r3,type = "queen",torus = F)    
...
#abline(v=quantile(Z1.plot,0.95),col="magenta",lwd=2)

par(new=TRUE)
fbplot(den.1,method = "MBD",ylim = c(0,4),xaxt = 'n',color=NA,outliercol=NA,barcol="orange3",ylab=" ",xlab=" " )

Z11.t.plot<-(z1.2.t-mean(z1.2.t))
my.hist<-hist(Z11.t.plot,freq = F, breaks = 80, xlim=c(-0.99,0.99),
              main=" ",
              xlab=" ", col="gray", ylim = c(0,4))

Z11.t6.plot<-(z1.2.t6-mean(z1.2.t6))
my.hist<-hist(Z11.t6.plot,freq = F, breaks = 80, xlim=c(-0.99,0.99),
              main=" ",
              xlab=" ", col="gray", ylim = c(0,4))
####################### KERNEL DENSITY

ks.den<-density(Z11.plot,bw=0.165)

lines(ks.den$x,ks.den$y,ylim=c(0,3))
...
CDF.EMP1 <- ecdf((z1.2-mean(z1.2))/(1))
...
cdf.sad1 <- NULL
for(i in 1:length(seq.b)){
  cdf.emp1[i] <- CDF.EMP1(seq.b[i])
  cdf.sad1[i] <- CDF.SAD1(seq.b[i])
  
  
}
cdf.asy1 <- pnorm(theta.grid,0,sqrt(asy.sigma2))

plot(cdf.sad1,typ="l",col="blue")
lines(cdf.emp1)



plot(cdf.emp1,cdf.sad1,col="blue",typ = "l",lwd=2,main="",
     xlab="P(Empirical)",ylab="P(Models)")
lines(cdf.emp1,cdf.asy1,col="red",type = "l",lty=2,lwd=2)
abline(a=0,b=1,lwd=3,type = "l",lty=4)
...
smooth <- smooth.spline(seq.c,abs(rela.lug),df=16)
smooth1 <- smooth.spline(seq.c,rela.edg,df=20)
smooth2 <- smooth.spline(seq.c,abs(rela.boo),df=16)
plot(smooth1,col="dark green",pch=18,  lwd=2, ylim = c(-1.5,2),xlab = "b",
     ylab = "Relative error",main="")
lines(smooth,col="blue",lwd=2)
lines(seq.c,abs(rela.asy),col="red",type="l",lty=2,  lwd=2)
lines(smooth2,col="purple",lwd=4,lty=3)
...
lines(seq.b,cdf.emp1,lwd=3,type = "l",lty=4)
lines(seq.b,cdf.sad1,col="blue",lwd=2)
