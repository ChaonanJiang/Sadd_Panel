...
Z11.plot<-(z11.3-mean(z11.3))
my.hist<-hist(Z11.plot,freq = F, 
              breaks = 80,ylim = c(0,8.4), xlim=c(-1,1),
              main="",
              xlab="", col="gray")
...
CDF.SAD1 <- function(b){
  ...
  c<-sum(p)*diff(theta.grid)[1]
  return(c)
  
}

CDF.EMP1 <- ecdf((z11.3-mean(z11.3)))
...
plot(cdf.emp1,cdf.sad1,col="blue",typ = "l",lwd=2,main="",
     xlab="P(Empirical)",ylab="P(Models)",ylim=c(0,1))
