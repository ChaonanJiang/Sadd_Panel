...
n1 <- r2*r2   #n is 10000 
N1 <- n1*T 
W1 <- cell2nb(r2,r2,type = "queen",torus = T)    
...
n2 <- r3*r3   #n is 10000 
N2 <- n2*T 
W2 <- cell2nb(r3,r3,type = "queen",torus = T)    
...
Z1.plot<-(z1.3-mean(z1.3))
my.hist<-hist(Z1.plot,freq = F, breaks = 80, xlim=c(-1,1),
              main="",
              xlab="", col="gray", ylim = c(0,4.2))
plot(my.hist$breaks,
     c(my.hist$density,0)
     ,type="s",xlim=c(-0.99,0.99),ylim = c(0,2),lwd=2,xlab=" ", ylab="Density", main=" ",col="gray52")

# theta.grid<-my.hist$breaks[2:14]
# my.hist.dens<-my.hist$density


lines(theta.grid,p.std,typ='l',col="blue",ylim = c(0,3),lwd=2)

par(new=TRUE)
fbplot(den.1,method = "MBD",ylim = c(0,2),xaxt = 'n',color=NA,outliercol=NA,barcol="orange3",ylab=" ",xlab=" " )
