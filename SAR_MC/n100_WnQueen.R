...
set.seed(1234)
boot.times <- 5000
lambda.hat1 <- vector(length = boot.times*1000)
for (j in 1:1000) {
  W <- cell2nb(r1,c1,type = "queen",torus = F)    
  Wl <- nb2listw(W)   #listw object 
  Wn <- listw2dgCMatrix(Wl)  #sparse matrix
...
Z11.plot<-(z11.2-mean(z11.2))
my.hist<-hist(Z11.plot,freq = F, 
              breaks = 80,ylim = c(0,7.5), xlim=c(-1,1),
              main="",
              xlab="", col="gray")
...
lines(theta.grid,p.std,typ='l',col="blue",ylim = c(0,7.5),lwd=2)
...
ks.den<-density(Z11.plot,bw=0.2)
...
CDF.EMP1 <- ecdf((z11.2-mean(z11.2)))
...
#lines(cdf.emp1,cdf.boo,col="purple",lwd=4,lty=3)
