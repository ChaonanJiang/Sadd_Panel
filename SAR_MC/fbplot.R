...
fbplot=function(fit,x=NULL,method='MBD',depth=NULL,plot=TRUE,prob=0.5,color=6,outliercol=2,
  ...
  if (length(depth)==0){
	...
	dp_s=rev(sort(depth,decreasing=F))
	...
	if (plot) {
	plot(x,y,pch=3,col=1,cex=0.4,type='p',xlim,ylim,...)
	}
	for (pp in 1:length(prob)){
		...
		if (prob[pp]==0.5){ #check outliers
			...
			if (plot) {
			#lines(c(x[bar],x[bar]),c(maxcurve[bar],sup[bar]),col=barcol,lwd=2,lty=6)
		    #lines(c(x[bar],x[bar]),c(mincurve[bar],inf[bar]),col=barcol,lwd=2,lty=6)
			}
		}
		...
		if (plot) {
		if (prob[pp]==0.5) {
		  lines(x,inf,lty=6,lwd=2,col=barcol)
		  lines(x,sup,lty=6,lwd=2,col=barcol)
		  #polygon(xx,yy,col=color[pp],border=barcol,lty=6,lwd=2)
		  }
		else {polygon(xx,yy,col=color[pp],border=NA)}
		}
	}
	if (plot) {
	lines(x,fit[,index[1]],pch=3,col=1,cex=0.4,type='p')
	#lines(x,maxcurve,col=barcol,lty=6,lwd=2)
	#lines(x,mincurve,col=barcol,lty=6,lwd=2)
	if (fullout) {
		if (sum(outly)>0){
				if (plot) {
				matlines(x,out,lty=2,col=outliercol,type='l',...)
		...
		return(list(depth=depth,outpoint=outpoint,medcurve=which(med),first_quantile=inf,third_quantile=sup,median=fit[,index[1]]))
}
