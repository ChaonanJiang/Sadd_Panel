...
K.sup <- function(lambda, theta2){
  nlm <- nlm(f= K.psi.neg1, p = c(0,0), lambda = lambda, theta2 = theta2, ndigit = 2, gradtol = 1e-1,stepmax = 0.099,steptol=1e-1)
...
for (k in 1:K.size) {
  ...
  para <- nlminb(start = c(0,0.1), objective = log.lik.neg, lower = c(-0.99,-0.99), upper = c(0.99,0.99),control = list(rel.tol=1e-2,step.max= 0.06,eval.max=10))$par
  lambda.hat[k] <- para[1]
  rho.hat[k] <- para[2]
...
K.sup <- function(lambda, theta2){
  nlm <- nlm(f= K.psi.neg1, p = c(0,0), lambda = lambda, theta2 = theta2, ndigit = 2, gradtol = 1e-1,stepmax = 0.13,steptol=1e-1)
  # nlm <- optim(par = c(0,0),fn = K.psi.neg,  lambda = lambda, theta2 = theta2, method = "BFGS")
  #nlm <- nlminb(start = c(0,0), objective = K.psi.neg,lambda = lambda, theta2 = theta2,control = list(rel.tol=1e-6, step.min=1e-4,step.max= 0.5))
  k.sup <- -nlm$minimum
  nu.est <- nlm$estimate
...
K.inf <- function(lambda){
  #return(optimize(K.sup,interval=c(-0.99,0.99), lambda = lambda)$objective)
  #return(optim(fn= K.sup, par = 0, lambda = lambda, lower = -0.99, upper = 0.99, method = "L-BFGS-B")$value)
  f <- nlminb(start = 0, objective = K.sup, lambda = lambda, lower = -0.99, upper = 0.99,control = list(rel.tol=1e-2, step.min=0.01,step.max= 0.08,eval.max=10))
  k.inf <- f$objective
  theta2.est <- f$par
  # seq.theta2 <- seq(-0.9,0.9,0.1)
  # k.start <- K.sup(lambda, seq.theta2[1])
...
K.sup <- function(lambda, theta2){
  nlm <- nlm(f= K.psi.neg1, p = c(0,0), lambda = lambda, theta2 = theta2, ndigit = 2, gradtol = 1e-1,stepmax = 0.0285,steptol=1e-1)
  # nlm <- optim(par = c(0,0),fn = K.psi.neg,  lambda = lambda, theta2 = theta2, method = "BFGS")
  #nlm <- nlminb(start = c(0,0), objective = K.psi.neg,lambda = lambda, theta2 = theta2,control = list(rel.tol=1e-6, step.min=1e-4,step.max= 0.5))
  k.sup <- -nlm$minimum
  nu.est <- nlm$estimate
...
K.inf <- function(lambda){
  #return(optimize(K.sup,interval=c(-0.99,0.99), lambda = lambda)$objective)
  #return(optim(fn= K.sup, par = 0, lambda = lambda, lower = -0.99, upper = 0.99, method = "L-BFGS-B")$value)
  f <- nlminb(start = 0, objective = K.sup, lambda = lambda, lower = -0.99, upper = 0.99,control = list(rel.tol=1e-2, step.min=0.01,step.max= 0.25,eval.max=10))
  k.inf <- f$objective
  theta2.est <- f$par
  # seq.theta2 <- seq(-0.9,0.9,0.1)
  # k.start <- K.sup(lambda, seq.theta2[1])
...
for (k in 1:K.size) {
  ...
  para <- nlminb(start = c(0,0.1), objective = log.lik.neg, lower = c(-0.99,-0.99), upper = c(0.99,0.99),control = list(rel.tol=1e-2,step.max= 0.08,eval.max=10))$par
  lambda.hat[k] <- para[1]
  rho.hat[k] <- para[2]
...
c(pchisq(t_s_095025,1),pchisq(t_s_0975025,1))
c(pchisq(t_wald095025,1),pchisq(t_wald0975025,1))
...
c(pchisq(t_s_09505,1),pchisq(t_s_097505,1))
c(pchisq(t_wald09505,1),pchisq(t_wald097505,1))


#c(t_s_095075,t_s_0975075)
#c(t_wald095075,t_wald0975075)
c(pchisq(t_s_095075,1),pchisq(t_s_0975075,1))
c(pchisq(t_wald095075,1),pchisq(t_wald0975075,1))
