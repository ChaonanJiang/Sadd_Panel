...
######### LOAD the MC results for MLE, z1.3 data from SAR_mle.rdata
load("SAR_mle.rdata")
...
# (dotted line) probability approximation for n=24 and Wn=queen with torus.   #
###############################################################################
...
###############################################################################
# Figure 5 in Appendix D.4: Relative error (in absolute value) for the approximate right tail #
...
for(i in 1:length(seq.c)){
  rela.sad[i] <- (1-CDF.SAD1(seq.c[i]))/(1-CDF.EMP(seq.c[i]))-1
