...
#   Cn0 <- rnorm(n)
#   Xn <- rnorm(n*T)
#   V <- rnorm(n*T,0,sqrt(sig2))
#   Yn <- as.matrix(solve((Diagonal(n*T)-kronecker(Diagonal(T),lambda0*Wn.a)))%*%(Xn*beta + rep(Cn0,T) + solve((Diagonal(n*T)-kronecker(Diagonal(T),rho*Mn.a)))%*%V))
#   
#   dt1 <- data.frame(id = rep(c(1:n),T), time =kronecker(1986:2000,rep(1,24)),Yn,Xn)
#   
#   Y.tilde.nt<-matrix(Yn,nrow=n,ncol=T)-matrix(rep(rowMeans(matrix(Yn,nrow=n,ncol=T)),T),n,T)
#   X.tilde.nt<-matrix(Xn,nrow=n,ncol=T)-matrix(rep(rowMeans(matrix(Xn,nrow=n,ncol=T)),T),n,T)
#   V.tilde.nt <- matrix(V,nrow=n,ncol=T)-matrix(rep(rowMeans(matrix(V,nrow=n,ncol=T)),T),n,T)
#   sarar1 <- spml(formula = Yn~Xn, data = dt1 , listw = mat2listw(W), model = "within", spatial.error= "b",lag = T, LeeYu = T, Hess = F)
#   summary(sarar1)
...
M <- M.matrix1(beta,lambda0,rho,sig2)
...
for(i in 1:n){
  s <- s+ der2.Score.m1(i,beta,lambda0,rho,sig2)
}
der2.Score1 <- s/n
#####2nd 
...
for(i in 1:n){
  s <- s+ der2.Score.m2(i,beta,lambda0,rho,sig2)
}
der2.Score2 <- s/n
#####3rd 
...
for(i in 1:n){
  s <- s+ der2.Score.m3(i,beta,lambda0,rho,sig2)
}
der2.Score3 <- s/n
#####4th 
...
for(i in 1:n){
  s <- s+ der2.Score.m4(i,beta,lambda0,rho,sig2)
}
der2.Score4 <- s/n
###### Gamma function 
...
G.powers<- function(beta,lambda,rho,sig2){
  ...
  for(j in 1:MCsize){
    ...
    for(i in 1:n){
      ...
      g1[i] <- t(solve(M)%*%b)%*%c(0,1,0,0)
      g2[i] <- g1[i]*g1[i]
      g3[i] <- g2[i]*(t(solve(M)%*%b)%*%c(0,1,0,0))
      g4[i] <- g3[i]*(t(solve(M)%*%b)%*%c(0,1,0,0))
...
g.powers <- G.powers(beta,lambda0,rho,sig2)
  ...
  for(k in 1:MCsize1){
    ...
    for(i in 1:n){
      for(j in 1:n){
        ...
        gamm1[i,j] <- (IF.i+IF.j+M.inv%*%Gamma.ij+M.inv%*%(Sum.Der1.j%*%IF.i+Sum.Der1.i%*%IF.j) )[2]
        gamm2[i,j] <- gamm1[i,j]*gamm1[i,j]
        gamm3[i,j] <- gamm1[i,j]*IF.i[2]*IF.j[2]
        gamm4[i,j] <- gamm3[i,j]*IF.i[2]
      }
    }
...
gam.con1 <- Gam.Con1(beta,lambda0,rho,sig2)
    ...
    for(i in 1:n){
      for(j in 1:n){
        for(l in 1:n){
          ...
          gam.il <- (IF.i+IF.l+M.inv%*%Gamma.il +M.inv%*%(Sum.Der1.l%*%IF.i+Sum.Der1.i%*%IF.l) )[2]
          ...
          gamma.4j <- t(IF.l)%*%der2.Score.m4(i,beta,lambda,rho,sig2)%*%IF.j
          Gamma.jl <- c(gamma.1j,gamma.2j,gamma.3j,gamma.4j)
          gam.jl <- (IF.j+IF.l+M.inv%*%Gamma.jl +M.inv%*%(Sum.Der1.l%*%IF.j+Sum.Der1.j%*%IF.l) )[2]
          
          gamm1 <- gamm1+IF.i[2]*IF.j[2]*gam.il*gam.jl
...
gam.vec <- foreach(i = 1:MC.size,.combine='c')%dopar%{
  library("spdep") 
  library("splm") 
  gamm1 <- 0
  V.tilde <- matrix(rnorm(n*T,mean = 0, sd = sqrt((T-1)*sig2/T)),n,T)
  ...
  for(i in 1:n){
    for(j in 1:n){
      for(l in 1:n){
        ...
        ###### j,l
        gamma.1j <- t(IF.l)%*%der2.Score.m1(i,beta,lambda0,rho,sig2)%*%IF.j
        gamma.2j <- t(IF.l)%*%der2.Score.m2(i,beta,lambda0,rho,sig2)%*%IF.j
        gamma.3j <- t(IF.l)%*%der2.Score.m3(i,beta,lambda0,rho,sig2)%*%IF.j
        gamma.4j <- t(IF.l)%*%der2.Score.m4(i,beta,lambda0,rho,sig2)%*%IF.j
        Gamma.jl <- c(gamma.1j,gamma.2j,gamma.3j,gamma.4j)
        gam.jl <- (IF.j+IF.l+M.inv%*%Gamma.jl +M.inv%*%(Sum.Der1.l%*%IF.j+Sum.Der1.j%*%IF.l) )[2]
...
cum4.nt <- (sqrt(g.powers[2]))^(-4)*(g.powers[4] +12*gam.con1[4]+12*gam.pwr1)-3 
...
1-CDF.SAD1(round(sarar$coefficients[1],4)) 
...
Asy.sig2 <- solve(SIGMA(beta,lambda,rho,sig2))[2,2]/m
   ### 0.1918893 
1-pnorm(sarar1$coefficients[1],mean=0,sd=sqrt(Asy.sig2))