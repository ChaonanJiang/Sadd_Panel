...
cum4.nt <- (g.powers[2])^(-2)*(g.powers[4] +12*gam.con1[4]+12*Gam.Con2(beta0,lambda,rho,sig2))-3 
...
for(i in 1:length(seq.c)){
  der2.cgf.w[i] <- Der2.cgf.Wang(seq.c[i],2.485)
}
...
Der1.cgf.Wang <- function(u){
  wn <- exp(-n*Sigma2.nT*2.485*2.485*u*u/2) 
  der1 <-  (gam.con1[1]/n + n*Sigma2.nT*u +((1/2)*(n^(1.5))*cum3.nt*(Sigma3.nT)*(u*u)+(1/6)*(n*n)*cum4.nt*(Sigma4.nT)*(u*u*u))*wn+
            ((1/6)*(n^(1.5))*cum3.nt*(Sigma3.nT)*(u*u*u)+(1/24)*(n*n)*cum4.nt*(Sigma4.nT)*(u*u*u*u))*wn*(-n*Sigma2.nT*2.485*2.485*u))
  return(der1)
}
...
Sad <- function(a){
  ...
  sad <- uniroot(function(u) Der1.cgf.Wang(u)-a, lower = -1000,upper = 1000)$root
...
sad.grid<-seq(-0.9999,0.9999,0.0001)
sad.vals<-NULL
...
p.nT <- function(a){
  p <- sqrt(n/(2*pi*Der2.cgf.Wang(Sad(a),2.485)))*exp(n*(cgf(Sad(a))-a*Sad(a)))
  return(p)
}
