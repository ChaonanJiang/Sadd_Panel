...
for(i in 1:length(seq.c)){
  der2.cgf.w[i] <- Der2.cgf.Wang(seq.c[i],0.5562)
}
...
Der1.cgf.Wang <- function(u){
  wn <- exp(-n*Sigma2.nT*0.5562*0.5562*u*u/2) 
  der1 <-  (gam.con1[1]/n + n*Sigma2.nT*u +((1/2)*(n^(1.5))*cum3.nt*(Sigma3.nT)*(u*u)+(1/6)*(n*n)*cum4.nt*(Sigma4.nT)*(u*u*u))*wn+
              ((1/6)*(n^(1.5))*cum3.nt*(Sigma3.nT)*(u*u*u)+(1/24)*(n*n)*cum4.nt*(Sigma4.nT)*(u*u*u*u))*wn*(-n*Sigma2.nT*0.5562*0.5562*u))
  return(der1)
}
...
Sad <- function(a){
  ...
  sad <- uniroot(function(u) Der1.cgf.Wang(u)-a, lower = -10000,upper = 10000)$root
...
p.nT <- function(a){
  p <- sqrt(n/(2*pi*Der2.cgf.Wang(Sad(a),0.5562)))*exp(n*(cgf(Sad(a))-a*Sad(a)))
  return(p)
}
