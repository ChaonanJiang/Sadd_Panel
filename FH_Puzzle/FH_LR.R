...
###### c.g.f. K ########
K.psi.neg <- function(nu, theta1, theta2){  ## consider a simple case theta2 = rho
  beta <- theta2[1] 
  sig2 <- theta2[2]
 ...
 Vnt <- matrix(rnorm(N*MC.size,sd = sqrt(sig2)),nrow = N, ncol = MC.size)
  for (i in 1:n) {
    ...
    for (j in 1:MC.size) {
      ...
      Yn1 <- solve((diag(N)-kronecker(diag(T),lambda0*Wn.a)))%*%(rep(Cn0[,j],T) + Xn*beta + solve((diag(N)-kronecker(diag(T),rho0*Mn)))%*%Vnt[,j])
...
K.psi.neg1 <- function(nu, theta1, theta2){  
  beta <- theta2[1] 
  sig2 <- theta2[2]
...
###### inf sup K #######
set.seed(1234)
# start.value <- runif(4,-0.1,0.1)
K.sup <- function(theta1, theta2){
    nlm <- nlm(f= K.psi.neg1, p = c(0,0,0,0), theta1 = theta1, theta2 = theta2, stepmax = 1, steptol = 0.01,iterlim = 50,gradtol = 1e-3)
  # nlm <- optim(par = start.value,fn = K.psi.neg1,  lambda = lambda, theta2 = theta2, method = "BFGS")
  #  nlm <- nlminb(start = start.value, objective = K.psi.neg1,lambda = lambda, theta2 = theta2, control = list(rel.tol= 1e-2,iter.max=50,eval.max = 50))
  ...
  #k.sup <- -nlm$objective
  #nu.est <- nlm$par
  print(c(nu.est,k.sup))
  return(k.sup)  # add minus sign
}


K.inf <- function(theta1){
  f <- nlminb(start = c(beta.hat,sig2.hat), objective = K.sup, theta1 = theta1,
              lower = c(-9999,0.001), upper = c(9999,9999),
              control = list(rel.tol= 1e-2,iter.max=50,eval.max=50 ))
  k.inf <- f$objective
  theta2.est <- f$par
  # f <- optim(par = c(0,0,0.01), fn = K.sup,lambda= lambda, method = "L-BFGS-B", lower = c(-9999,-0.99,0.001), upper = c(9999,0.99,9999))
  # k.inf <- f$value
  # theta <- f$par
...
cov <- (solve(SIGMA(beta.hat,lambda.hat,rho.hat,sig2.hat)))/m
theta1.cov <- matrix(c(cov[2,2],cov[2,3],cov[3,2],cov[3,3]),2,2)
theta1.hat <-c(lambda.hat,rho.hat) 
test.wald <- t(theta1.hat)%*%solve(theta1.cov)%*%theta1.hat
print(test.wald)

ptm <- proc.time()

k.inf <- K.inf(theta1.hat)
h.hat <- k.inf[1]
theta2.est <- k.inf[-1]
test.stat <- 2*n*h.hat 
