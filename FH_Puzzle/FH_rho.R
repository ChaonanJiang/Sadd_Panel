##################################################################
############# Test for the null hypothese rho = 0 ################
##################################################################
library("spdep") 
library("splm") 
library("graphics")
...
###### c.g.f. K ########
K.psi.neg <- function(nu, rho, theta2){  ## consider a simple case theta2 = rho
  beta <- theta2[1] 
  lambda <- theta2[2]
  sig2 <- theta2[3]
 ...
 Vnt <- matrix(rnorm(N*MC.size,sd = sqrt(sig2)),nrow = N, ncol = MC.size)
  for (i in 1:n) {
    ...
    for (j in 1:MC.size) {
      ...
      Yn1 <- solve((diag(N)-kronecker(diag(T),lambda*Wn.a)))%*%(rep(Cn0[,j],T) + Xn*beta + solve((diag(N)-kronecker(diag(T),rho0*Mn)))%*%Vnt[,j])
...
K.psi.neg1 <- function(nu, rho, theta2){  
  beta <- theta2[1] 
  lambda <- theta2[2]
  sig2 <- theta2[3]
  Sn <- diag(n)-lambda*Wn.a
...
K.sup <- function(rho, theta2){
    nlm <- nlm(f= K.psi.neg1, p = c(0,0,0,0), rho = rho, theta2 = theta2, stepmax = 1, steptol = 0.01,iterlim = 50,gradtol = 1e-3)
...
K.inf <- function(rho){
  f <- nlminb(start = c(beta.hat,lambda.hat,sig2.hat), objective = K.sup, rho=rho,
              lower = c(-9999,-0.99,0.001), upper = c(9999,0.99,9999),
              control = list(rel.tol= 1e-3,iter.max=50,eval.max=50 ))
  k.inf <- f$objective
  theta2.est <- f$par
...
lamb.cov <- (solve(SIGMA(beta.hat,lambda.hat,rho.hat,sig2.hat)))[3,3]/m
test.wald <- (rho.hat-rho0)^2/lamb.cov
