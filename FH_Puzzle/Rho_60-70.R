...
Der1.cgf.Wang <- function(u){
  wn <- exp(-n*Sigma2.nT*0.5*0.5*u*u/2) 
  der1 <-  (gam.con1[1]/n + n*Sigma2.nT*u +((1/2)*(n^(1.5))*cum3.nt*(Sigma3.nT)*(u*u)+(1/6)*(n*n)*cum4.nt*(Sigma4.nT)*(u*u*u))*wn+
              ((1/6)*(n^(1.5))*cum3.nt*(Sigma3.nT)*(u*u*u)+(1/24)*(n*n)*cum4.nt*(Sigma4.nT)*(u*u*u*u))*wn*(-n*Sigma2.nT*0.5*0.5*u))
  return(der1)
}
###### find saddlepoints
Sad <- function(a){
...
# asymptotic (ASY) approximation between 1960-1970 for MLE rho hat.         #
#############################################################################
