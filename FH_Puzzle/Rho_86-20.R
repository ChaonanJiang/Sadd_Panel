...
M <- M.matrix1(beta,lambda,rho0,sig2)
...
for(i in 1:n){
  s <- s+ der2.Score.m1(i,beta,lambda,rho0,sig2)
}
der2.Score1 <- s/n
#####2nd 
...
for(i in 1:n){
  s <- s+ der2.Score.m2(i,beta,lambda,rho0,sig2)
}
der2.Score2 <- s/n
#####3rd 
...
for(i in 1:n){
  s <- s+ der2.Score.m3(i,beta,lambda,rho0,sig2)
}
der2.Score3 <- s/n
#####4th 
...
for(i in 1:n){
  s <- s+ der2.Score.m4(i,beta,lambda,rho0,sig2)
}
der2.Score4 <- s/n
###### Gamma function 
...
g.powers <- G.powers(beta,lambda,rho0,sig2)
...
gam.con1 <- Gam.Con1(beta,lambda,rho0,sig2)
...
gam.vec.1 <- foreach(i = 1:MC.size,.combine='c')%dopar%{
  library("spdep") 
  library("splm") 
  gamm1 <- 0
  V.tilde <- matrix(rnorm(n*T,mean = 0, sd = sqrt((T-1)*sig2/T)),n,T)
  ...
  for(i in 1:n){
    for(j in 1:n){
      for(l in 1:n){
        ...
        ###### j,l
        gamma.1j <- t(IF.l)%*%der2.Score.m1(i,beta,lambda,rho0,sig2)%*%IF.j
        gamma.2j <- t(IF.l)%*%der2.Score.m2(i,beta,lambda,rho0,sig2)%*%IF.j
        gamma.3j <- t(IF.l)%*%der2.Score.m3(i,beta,lambda,rho0,sig2)%*%IF.j
        gamma.4j <- t(IF.l)%*%der2.Score.m4(i,beta,lambda,rho0,sig2)%*%IF.j
        Gamma.jl <- c(gamma.1j,gamma.2j,gamma.3j,gamma.4j)
...
gam.pwr.1<- sum(gam.vec.1)/MC.size
gam.pwr1 <- gam.pwr.1/(16*n*n*n)
...
seq.b <- seq(-5,5,0.0001)
a <- seq(2.75,2.85,0.0001)
der2.cgf.w <- vector(length = length(seq.b))
a.t <- NULL
k <- 1 
for (j in 1:length(a)) {
  for(i in 1:length(seq.b)){
    der2.cgf.w[i] <- Der2.cgf.Wang(seq.b[i],a[j])
  }
  if (min(der2.cgf.w) > 0.0000001) {
    a.t[k] <- a[j]
    k <- k+1
  }
...
seq.c <- seq(-15,15,0.1)
der2.cgf.w <- vector(length = length(seq.c))
for(i in 1:length(seq.c)){
  der2.cgf.w[i] <- Der2.cgf.Wang(seq.c[i],2.8146)
}
plot(seq.c,der2.cgf.w)
abline(h=0)


Der1.cgf.Wang <- function(u){
  wn <- exp(-n*Sigma2.nT*2.8146*2.8146*u*u/2) 
  der1 <-  (gam.con1[1]/n + n*Sigma2.nT*u +((1/2)*(n^(1.5))*cum3.nt*(Sigma3.nT)*(u*u)+(1/6)*(n*n)*cum4.nt*(Sigma4.nT)*(u*u*u))*wn+
              ((1/6)*(n^(1.5))*cum3.nt*(Sigma3.nT)*(u*u*u)+(1/24)*(n*n)*cum4.nt*(Sigma4.nT)*(u*u*u*u))*wn*(-n*Sigma2.nT*2.8146*2.8146*u))
  return(der1)
}
###### find saddlepoints
Sad <- function(a){
...
# asymptotic (ASY) approximation between 1986-2000 for MLE rho hat.         #
#############################################################################
...
1-CDF.SAD1(round(sarar$coefficients[2],4))   ####  sparse weights matrix:0.0455737, Wdia1: multiple solutions of sad
...
Asy.sig2 <- solve(SIGMA(beta,lambda,rho,sig2))[3,3]/m
1-pnorm(sarar$coefficients[2],mean=0,sd=sqrt(Asy.sig2))   ### 0.04464198 
